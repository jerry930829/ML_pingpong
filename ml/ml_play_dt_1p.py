"""
Decision-tree based MLPlay for 1P. Loads model from ml/models/dt_1p.joblib
"""
import os
import joblib
import pandas as pd
...
MODEL_PATH = os.path.join(os.path.dirname(__file__), 'models', 'dt_1p.joblib')
...
class MLPlay:
    def __init__(self, ai_name, *args, **kwargs):
        ...
        if not self.features:
            # fallback to hard-coded features
    ...
    def update(self, scene_info, *args, **kwargs):
        if scene_info['status'] != 'GAME_ALIVE':
            return 'RESET'
        # Maintain a small history buffer on the agent for time-series features
        if not hasattr(self, 'history'):
            from collections import deque
            self.history = deque(maxlen=2)

        # append a minimal snapshot of scene_info
        self.history.append({
        ...
        # previous-frame features default to 0
        ...
        # Build DataFrame with feature names to avoid sklearn warning
        df = pd.DataFrame([vals], columns=self.features)
        pred = self.clf.predict(df)[0]
        action = self.int_to_action[int(pred)]

        # Same post-processing as RF 1P: landing-based conservative moves + smoothing
