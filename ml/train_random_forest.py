"""
Train RandomForest models for 1P and 2P from collected CSV data.
Saves models to ml/models/rf_1p.joblib and rf_2p.joblib
"""
import os
import pandas as pd
...
# Feature order used for training — match train_decision_tree.py
...
def train_for(player_side='1P', n_estimators=100, max_depth=12):
    df = pd.read_csv(DATA_CSV)
    df = df[df['player'] == player_side]
    if df.empty:
        raise RuntimeError(f'No data for {player_side} in {DATA_CSV}')

    # Pick available features (backwards-compatible)
...
def train_with_params(player_side='1P', n_estimators=100, max_depth=12, balance=False, class_weight=False):
    X_train, X_test, y_train, y_test = train_for(player_side, n_estimators, max_depth)

    # Handle simple over-sampling to balance classes on training set
    if balance:
    ...
    print(f'---- {player_side} RandomForest model ----')
    ...
    model_path = os.path.join(MODELS_DIR, f'rf_{player_side.lower()}.joblib')
...
if __name__ == '__main__':
    ...
    train_with_params('1P', n_estimators=args.n_estimators, max_depth=args.max_depth, balance=args.balance, class_weight=args.class_weight)
    train_with_params('2P', n_estimators=args.n_estimators, max_depth=args.max_depth, balance=args.balance, class_weight=args.class_weight)
